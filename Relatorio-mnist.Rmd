...
ids <- sample(nrow(db_digitos), size = .05*nrow(db_digitos), replace = FALSE)
```
...
# resultados <- rbind(resultados,data.frame(modelo="Floresta aleatoria",erro_dentro=rf_ac_dentro, erro_fora=rf_ac_fora))
resultados <- resultados %>% mutate(across(where(is.numeric),round,4))
resultados
