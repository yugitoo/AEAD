...
cv_lasso <- cv.glmnet(X[ids,], as.factor(db_digitos$y_treino[ids]), family = "multinomial", alpha = 1,
                      type.measure = "class", trace.it = TRUE, nlambda = 30, maxit = 10000)
...
resultados <- rbind(resultados,data.frame(modelo="Lasso - Lambda 1 desvio padrao",acuracia_dentro=lasso_ac_dentro2, acuracia_fora=lasso_ac_fora2))
...
modelo_rn <- keras_model_sequential() %>%
  layer_dense(units = 100, # numero de unidades
              activation = "softmax", # funcao de ativacao
              input_shape = ncol(X)) %>% # dimensao da entrada
  layer_dropout(rate = 0.4) %>% # taxa de nos nos suprime em cada etapa
  layer_dense(units = 10) # camadas de saida com uma unica unidade pq eh um modelo de regressao
