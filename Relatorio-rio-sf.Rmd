...
cv_lasso5 <- cv.glmnet(X[ids,], treino_sf$Y[ids], alpha = 1,
                      nlambda=30)
# cv_lasso <- cv.glmnet(X[ids,], treino_sf$Y[ids], alpha = 1,
#                       nlambda=500)
...
Vamos considerar uma estrutura simples com apenas uma camada oculta e 41 unidades, por corresponder a 50% da quantidade de camadas de entrada. Foi considerado 40% de supressão.

Observando o gráfico do erro ao longo das epochs, optou-se por consider 150 epochs e tamanho de batch = 32.
...
Percebemos que dentre modelos, o que teve melhor desempenho tanto no erro dentro quanto no erro fora foi o modelo de floresta aleatória. Dessa forma, a previsão para os dados de teste foi feita utilizando o modelo de floresta aleatória.
