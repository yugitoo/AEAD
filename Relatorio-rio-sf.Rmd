...
O dataframe estações contém informações sobre cada uma das estações que aparecem no dataframe treino_sf. Essas informações estão disponibilizadas apenas a título de curiosidade e não devem ser utilizadas para construir os modelos preditivos. Por exemplo, latitude e longitude, além do tipo da estação (fluviométrica ou pluviométrica) e outros detalhes técnicos de cada uma delas. A tabela abaixo mostra 5 das estações.
...
### Lasso?
Vamos tentar rodar um LASSO
```{r}
set.seed(12345)
X <- model.matrix(Y ~ .,
                  data = treino_sf)[,-1] # X deve ser uma matrix (-1 para retirar o intercepto)
...
Para avaliar os modelos, será utilizado o erro absoluto médio (MAE).

```{r}
(lasso_erro_dentro <- mean(y_lasso_dentro - treino_sf$Y[ids]))

# (lasso_erro_fora <- mean((y_lasso_fora - treino_sf[-ids])))
```
