...
```{r}
lasso <- glmnet(X[ids,], treino_sf$Y[ids], alpha = 1, lambda = cv_lasso$lambda.min)
```

```{r}
saveRDS(lasso, "sf_lasso.rds")
```

```{r}
readRDS("sf_lasso.rds")
```
...
# names(resultados) <- c("modelo", "erro_dentro", "erro_fora")
# typeof(resultados[,2])
resultados <- rbind(resultados,data.frame(modelo="lasso",erro_dentro=lasso_erro_dentro, erro_fora=lasso_erro_fora))
resultados <- resultados %>% mutate(across(where(is.numeric),round,4))
resultados
```
...
```{r}
(rf_erro_dentro <- mean(abs(y_rf_dentro - treino_sf$Y[ids])))

(rf_erro_fora <- mean(abs((y_rf_fora - treino_sf$Y[-ids]))))
```
```{r}
# resultados <- data.frame("lasso",lasso_erro_dentro, lasso_erro_fora)
# names(resultados) <- c("modelo", "erro_dentro", "erro_fora")
...
(rn_erro_dentro <- mean(abs (treino_sf$Y[ids] - pred_rn_dentro)))
(rn_erro_fora <- mean(abs (treino_sf$Y[-ids] - pred_rn_fora)))
# atencao: os resultados variam um pouco pois
# os calculos sao feitos do SGD no python

(resultados <- rbind(resultados,
                     data.frame(modelo = "Rede Neural",
                                erro_dentro = rn_erro_dentro,
                                erro_fora=rn_erro_fora)))
